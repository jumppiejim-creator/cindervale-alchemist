...
**Status: CLOSED 2026-07-21 (except balance tuning) — see `faction_pairs_recon.md` (repo root).** Diagnosed and fully fixed in one day: harmony bonuses wired via `getFeatureVal` (A); vacuous `.every()` guarded (B); the 12 missing Tidecrest/Skyreach pairs authored — 21 pairs, 6/3/6/6 by zone, Grand Alliance achievable everywhere (C); Embassy list loc-filtered + cross-zone envoys blocked + sidebar gate fixed (D); reward strings and Grand Alliance popup reworded to the real wired effects (E-cheap); dead `dLv>=6` gates and stale energy check fixed (F). **Remaining:** the **balance flag** — per-pair bonuses stack uncapped (full 6-pair harmony at Diplomat Lv5 = 180% buy discount); tuning is Jim's call. The recon §6 table preserves the cut pair-specific bonus ideas if Jim ever wants them as real mechanics.
