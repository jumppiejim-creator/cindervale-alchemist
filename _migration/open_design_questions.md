...
**Status: CLOSED 2026-07-21 — see `faction_pairs_recon.md` (repo root).** Diagnosed and fully fixed in one day: harmony bonuses wired via `getFeatureVal` (A); vacuous `.every()` guarded (B); the 12 missing Tidecrest/Skyreach pairs authored — 21 pairs, 6/3/6/6 by zone, Grand Alliance achievable everywhere (C); Embassy list loc-filtered + cross-zone envoys blocked + sidebar gate fixed (D); reward strings and Grand Alliance popup reworded to the real wired effects (E-cheap); dead `dLv>=6` gates and stale energy check fixed (F); and the balance flag resolved via the tunable `HARMONY_CAPS` zone-total ceilings in game-data.js (restock 8 / discount 45% / rep +100% / sell +25%, applied after the Lv5 doubling — all zones reach identical capped maxima). Follow-up 2026-07-21: the §6 pair-specific bonuses were then BUILT as real tier-3 mechanics for all 9 original pairs (`pair.effect` blocks, exempt from doubling/caps — see the recon changelog). Only remaining idea: authoring 12 new pair-specific bonuses for the Tidecrest/Skyreach pairs, which currently carry the generic tier-3 reward.
