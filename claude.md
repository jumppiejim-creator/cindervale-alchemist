...
- **Open design questions** are consolidated in `_migration/open_design_questions.md` — literal Tier-3 planar interpretations, customer→faction mapping, and (new, July 21) item #9: Tinkerer `infiniteGadgetMarks` has no upgrade path and Runesmith named-weapon `goldValue` is dead data — both awaiting Jim's build-vs-reword pick. The guidance polish (#5), FACTION_PAIRS (#6), and per-level sufficiency expansion (#7) are closed.
