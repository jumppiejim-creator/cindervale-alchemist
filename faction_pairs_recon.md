...
- **2026-07-21 — Tidecrest/Skyreach pair-specific bonuses designed & built: all 21 pairs now have unique tier-3 rewards.**
  - Pure data authoring on the existing `pair.effect` infrastructure — zero new code. Each bonus merges its two factions' mechanical identities:
    - **Tidecrest:** Harbormasters↔Divers `+1 forage in Abyssal Trench & Coral Labyrinth` (the factions' signature unlock regions); Harbormasters↔Tidekeepers `reducedRisk 10%` (blessed, charted waters); Harbormasters↔Marine `caravan −1 day` (manifests cleared); Divers↔Tidekeepers `+15% rare finds` (the sea rewards the blessed harvest); Divers↔Marine `ingredients +2g each` (reef wealth reaches market); Tidekeepers↔Marine (DC13) `+10% all XP` (sacred trade routes teach).
    - **Skyreach:** Skywardens↔Starcallers `+15% quest gold` (vigil rewarded); Skywardens↔Traders `reducedRisk 10%` (guarded passes); Skywardens↔Windrunners (DC13) `armor enchants +25% gold` (deliberate armor-echo of Cindervale's DC13 weapon-gold pair); Starcallers↔Traders `+10% potion value` (starlight-blessed wares); Starcallers↔Windrunners `Celestial Ink +2 brew checks` (echo of Celestial Balm — the recipe is brewed from starcaller_seal); Traders↔Windrunners `caravan −1 day` (mirror of Ashfall's profit-and-exploration pair).
  - All keys verified live-wired (`rareForageBonus` at index.html:2226, `ingrSellBonus`, `xpMultiplier`, `questGoldBonus`, `potionValueBonus`, plus the recipeBonus/regionForage/caravan hooks from the §6 build). Within-zone key uniqueness verified — no same-key stacking inside a zone; cross-zone repeats (reducedRisk, caravanSpeedBonus) are deliberate rhymes, isolated by the loc filter.
  - Verified: `node -c` clean (4,969 lines); browser console sim — 21/21 pairs with effects, zero data problems, all 12 new bonuses resolve to designed values in their home zones, zero cross-zone leakage. Cache-bust → `?v=20260721d`.
  - **The FACTION_PAIRS system is now feature-complete:** every pair in every zone has real tiers 1-3, a unique tier-3 identity, capped zone totals, and a truthful UI.
