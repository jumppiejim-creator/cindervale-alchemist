...
- **2026-05-13** — Re-sample fix pass: closed 5 items found by accuracy re-sample audit. Wildcrafter L2 Fresh Reagents +1 ingredient wired into `_returnToTown` (picks a random rolled-this-trip ingredient and duplicates it; skip if `cumG` empty). Antiquarian L1 Relic Sense description trimmed to remove false "1 Energy" cost claim (`appraiseRelic` is genuinely free). Three Phase 3 terminology drift items ("per forage hour" / "per hour" / "per expedition hour" → "per forage roll" / "per forage roll" / "per forage roll") corrected across Cartographer L1, Warden L7, and Antiquarian L1. **Class feature audit closes at 140/140 ✓ accurate AND 140/140 🔓 exclusive.**
...
- **2026-07-21** — **Correction (FACTION_PAIRS recon, see `faction_pairs_recon.md`):** the Diplomat Lv3/Lv4/Lv5 harmony-bonus verdicts above cited `getHarmonyBonus()` as the wiring, but the function was **never called** — the audit verified its internals, not a call site, so the harmony percentage bonuses (per-pair shop/discount/rep, Grand Alliance +50%/+25%, Lv5 Ambassador doubling) were dead at runtime. Fixed same day: `getHarmonyBonus` relocated beside `getFeatureVal` and called from it, so all four effect keys now aggregate harmony everywhere; vacuous `.every()`-on-empty guarded at all four sites (pairless zones no longer get free Grand Alliance); two impossible `dLv>=6` gates corrected to `>=3`. The Lv4 legendary-stock and Lv5 repSpillover portions were genuinely live all along. Diplomat rows return to ✓ accurate in effect, with a caveat: per-pair stacking is uncapped (balance flag logged in the recon doc).
...
| 3 | Cartographer's Insight | **Familiar Territory** (forage DC -1 at regions visited 5+ times) + tile ingredient preview + +10% XP foraging | Phase D's `revealRegionIngredients` + `xpBonus:0.10` flags retained. **2026-05-13 redesign**: added new `familiarTerritory:true` flag; introduced `regionVisits` state (persisted in gatherState/restoreState); `doHarvest` increments visit counter on expedition entry; `_executeForageRoll` reduces `effectiveDC` by 1 when `prestigeLevels.cartographer>=3` AND `regionVisits[reg.id]>=5`. Tile ingredient preview remains as a convenience secondary affordance (acknowledged in description). | ✓ accurate | Redesigned 2026-05-13 — exclusivity follow-up. Replaces the redundant tile-preview claim with a genuinely exclusive depth-rewarding DC reduction. |
...
| Level | Feature | Handbook says | Code does | Verdict | Notes |
|-------|---------|---------------|-----------|---------|-------|
| 1 | Embassy | Double faction rep gains; +10 bonus rep per quest turn-in; **+1 daily rep at each Friendly+ faction** | `doubleRep` + `questRepBonus:10` wired via direct prestige gates per Phase B. **2026-05-13 redesign**: new `embassyPassiveRep:true` flag; `newDay` adds +1 rep to every faction with rep ≥ 150 when `prestigeLevels.diplomat>=1`. Result added to morning report as 🤝 Embassy Network section. Replaces dropped "unique NPC dialogue" claim. | ✓ accurate | Redesigned 2026-05-13 — exclusivity follow-up. Replaces unimplemented dialogue claim with concrete daily diplomatic tick. |
| 2 | Trade Agreements | +3 rare ingredients/day; +30% quest gold; **one Friendly+ faction's signature ingredient appears in daily shop** | `shopRestockBonus:3` + `questGoldBonus:0.30` wired via direct prestige gates per Phase B. **2026-05-13 redesign**: new `factionRestock:true` flag; `newDay` shop restock now appends one random Friendly+ faction's `res` ingredient (e.g. `sand_merchant_seal`, `diver_token`) to the procureItems list when `prestigeLevels.diplomat>=2`. Replaces dropped "exclusive vendors" and "cross-faction quests" claims. | ✓ accurate | Redesigned 2026-05-13 — exclusivity follow-up. Replaces unimplemented vendor/quest claims with a faction-pool restock mechanic using existing `FACTIONS[fid].res` data. |
...
| 4 | Grand Alliance | +50% rep + 25% sell when all harmonies maxed; **legendary-tier shop stock** | `getHarmonyBonus` delivers the percentage bonuses (line 3262-3278). **2026-05-13 redesign**: new `grandAllianceLegendaryStock:true` flag; `restockShop` (line 5037+) now appends one random `val>=25` ingredient to procureItems when `prestigeLevels.diplomat>=4` AND `FACTION_PAIRS.filter(fp=>(!fp.loc||fp.loc===gameLocation)).every(fp=>(factionHarmony[fp.id]||0)>=3)` — same allMax shape as the existing `getHarmonyBonus`. Pricing: `Math.ceil(val * 2.0)`. Pool: ~20 legendary ingredients across all 4 locations' endgame zones. Dedup against base shop + procList prevents duplicates. **Phase B fix preserved**: announcement gate at 3247 still corrected from `dLv>=10` to `dLv>=4`. | ✓ accurate | Redesigned 2026-05-13 — exclusivity follow-up. Closes the last partial feature from the exclusivity audit. Note: Tidecrest/Skyreach have no harmony pairs in FACTION_PAIRS data — `.every` on empty returns true, so Diplomat L4 chars in those locations get the legendary stock without harmony work (pre-existing behavior of `getHarmonyBonus`, not introduced here). |
...
- **✓ resolved 2026-05-13:** Picked option (b) — wired in `_returnToTown` right after the rareForageBonus block. New block picks a random ingredient already gathered this trip (filtering `cumG` keys for `cumG[id]>0`), duplicates it via `cumG[wcPick]++` + `modInv(wcPick,1)`, and emits a 🌿 Fresh Reagents log entry. Edge case: if `cumG` is empty (all rolls failed) the bonus skips — "bonus" implies a base to amplify. Mirrors the existing `guaranteedRare`/`rareForageBonus` patterns.
...
- **✓ resolved 2026-05-13:** Picked option (a) — description trimmed in `game-data.js` and handbook row updated. "1 Energy, Acumen check" → "free action, Acumen check". Handbook said "25 Energy" (pre-existing divergence between the two files); both now harmonized to "free action". Appraisal stays genuinely free, matching the relic system's narrative-action framing.
...
- **✓ resolved 2026-05-13:** Text replaced in `game-data.js` and `cindervale_handbook.html` row. "25% chance per forage hour" → "25% chance per forage roll". Mechanic unchanged.
...
- **✓ resolved 2026-05-13:** Text replaced in `game-data.js` and handbook row. "15% chance of bonus rare find per hour" → "15% chance of bonus rare find per forage roll". Mechanic unchanged.
...
- **✓ resolved 2026-05-13:** Bundled with #2's description trim. "15% relic find chance per expedition hour" → "15% relic find chance per forage roll" across `game-data.js` and the handbook row.
...
### Re-sample fix pass — closed 2026-05-13

All 5 findings from the re-sample are now resolved. Wildcrafter L2 wired (option b); Antiquarian L1 description trimmed (option a); 3 terminology-drift items text-replaced. **Audit re-closes cleanly at 140/140 ✓ accurate AND 140/140 🔓 exclusive.**
