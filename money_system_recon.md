...
- **2026-07-21 — Phase 4 complete: handbook pass DONE. PROJECT COMPLETE.**
  - **"Coinage of the Realm" sidebar added to Chapter 1** (house `note` box style): the 1pp = 10g = 100s standard, quoted-in-largest-coins convention, copper-as-street-change flavor, and the diegetic revaluation note for veterans ("every 'gold' in the old books was revalued as silver — your balance kept its full worth; only the labels changed").
  - **All literal amounts converted** to true denominated form: the settlement table now runs 3pp → 30pp; the 22-row upgrade table converted (5g … 3pp, with mixed forms like `1pp 5g`, `3g 5s`); room foundations, feat entries (+2s/+3s/+1s), Grand Collection +1g/morning, Fortified Walls +5g/morning, the Feywild 1g refund, Final Stand 80 gold, Torch table cap 15g, and the Shop Front tip recomputed (+5s/day — the eight-day payback math is unchanged).
  - **Decision-3 word pass applied** via the same phrase families as Phase 3 (+ per-site edits): "Gold Payment" → "Coin Payment", "Gold solutions" → "Coin solutions", Torch row "Gold" → "Coin", etc. Deliberate keeps: "Lead to Gold" spec name, "Gold speaks their language" (Caravan Kings flavor), the CSS `--gold` variable, and literal-gold flavor.
  - **Bonus fixes found in passing:** Chapter 1's Game Loop still described the pre-refactor "Energy (base 4) and Hours (base 4)" system — corrected to the shipped Energy-100 model, plus "spend hours extracting"/"per hour" → Energy/per-roll terminology.
  - Verified in the rendered handbook: sidebar + ratios + diegetic note render, stale "Hours (base 4)" gone, platinum-range settlement costs render, zero stale 3+-digit literals anywhere.
  - **The denominated money system is fully shipped: Phases 1-4 all landed 2026-07-21.** One integer wallet, zero save migration, ~250 display sites + all data/handbook text converted, lint-verified end to end.
